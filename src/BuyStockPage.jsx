...
const BuyStockPage = () => {
    ...
    const handleQuantityChange = (e) => {
        ...
        } else if (value * latestPrice > balance) {
            setError("Insufficient balance.");
        } else {
            setQuantity(value || 0);
    ...
    const handleBuy = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:5169/api/userprofile/buy?id=${userData.id}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Accept: 'application/json',
            ...
            if (!response.ok) {
                throw new Error(`Failed to buy stock: ${response.status}`);
            }
    ...
    return (
        <div className="container-fluid vh-100 p-0 investment-background">
            <header className="custom-header d-flex justify-content-between align-items-center shadow-sm" style={{ height: '80px' }}>
                ...
                />
                <h1 className="h2 ms-3 mb-0 text-white">Buy Stock: {companyName}</h1>
            ...
            <div className="text-end text-red pe-3 pt-2" style={{ position: 'absolute', top: 80, right: 0 }}>
                <strong>Purchase Expires In: {Math.floor(remainingTime / 60)}:{(remainingTime % 60).toString().padStart(2, '0')}</strong>
            </div>
            ...
            <main className="container py-5">
                ...
                <h3 className="text-center mb-4">Buy Stocks Form</h3>
                        ...
                        <div className="modal-dialog" role="document">
                            <div className="modal-content">
                                ...
                                <div className="modal-body">
                                    <p>Stock Purchased. Check Transaction Page For More Information.</p>
                                </div>
